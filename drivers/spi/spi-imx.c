			 ...
			 struct spi_transfer *transfer)
{
	...
	if (transfer && spi_imx->dma_is_inited &&
		...
		//printk("=====spi_imx_can_dma ok\n");
		return true;
	}
	//printk("=====spi_imx_can_dma fail,transfer(%08x),spi_imx->dma_is_inited(%d),transfer->len=%d,spi_imx_get_fifosize=%d\n",
	//	transfer,spi_imx->dma_is_inited,transfer->len,spi_imx_get_fifosize(spi_imx)
	//	);
	return false;
}
		...
		struct spi_imx_config *config)
{
	...
	//printk("spi_imx_config base:%08x: hz=%d,cs=%d,bpw=%d,mode=%d\n",spi_imx->base,config->speed_hz,config->cs,config->bpw,config->mode);
	return 0;
}
...
static irqreturn_t spi_imx_isr(int irq, void *dev_id)
{
	...
	while (spi_imx->devtype_data->rx_available(spi_imx)) {
		spi_imx->rx(spi_imx);
		spi_imx->txfifo--;
		//printk("0x%02x ",((uint8_t *)(spi_imx->rx_buf))[0]);
	}
				 ...
				 struct spi_transfer *t)
{
	...
	if (spi_imx->bitbang.master->can_dma &&
	   ...
	   // printk("===spi_imx_setupxfer->DMA config=====\n");
...
static void spi_imx_dma_rx_callback(void *cookie)
{
	...
	//printk("====spi_imx_dma_rx_callback===\n");
...
{
	struct spi_imx_data *spi_imx = (struct spi_imx_data *)cookie;
	//printk("====spi_imx_dma_tx_callback===\n");
				...
				struct spi_transfer *transfer)
{
	...
	if (tx) {
		...
		//printk("============spi_imx_dma_transfer txx \n");
	}
	...
	if (rx) {
		...
		//printk("spi_imx_dma_transfer rxx \n");
	}
	...
	if (!ret) {
		...
		ret = wait_for_completion_timeout(&spi_imx->dma_rx_completion,
				IMX_DMA_TIMEOUT(transfer->len));
		//printk("dma_rx_completion ret:%d,left:%d\n",ret,left);
		if (!ret) {
				...
				struct spi_transfer *transfer)
{
	...
	spi_imx->txfifo = 0;
	//printk("transfer:t%08x,r%08x,len:%d\n",transfer->tx_buf,transfer->rx_buf, transfer->len);
	for(i = 0;i <  transfer->len; i++){
		printk(" %02x",((uint8_t *)(transfer->tx_buf))[i]);
	}
	printk("\n");
	reinit_completion(&spi_imx->xfer_done);
...
static int spi_imx_probe(struct platform_device *pdev)
{
	...
	printk("spi_imx_probe base:%08x,master:%08x\n",spi_imx->base,spi_imx->bitbang.master);
...
//by whs20180511,通过master获得设备基地址，以便识别是哪一个SPI
void __iomem *spi_master_get_devbase(struct spi_master *master)
{
	struct spi_imx_data *spi_imx = spi_master_get_devdata(master);
	if(spi_imx){
		return spi_imx->base;
...
EXPORT_SYMBOL_GPL(spi_master_get_devbase);
